...
int produsScalarParallel(int a[MAX], int b[MAX]) {
    ...
    #pragma omp parallel firstprivate(partial_sum) num_threads(16) 
    {
        ...
        // private - se creeaza o copie locala threadului, nu i se copiaza valoarea
        // firstprivate - se creeaza o copie locala threadului, si i se pastreaza valoarea
...
int main()
{
    ...
    #pragma omp parallel num_threads(16) // incepe paralelismul, codul dintre {} va fi rulat in paralel, se creeaza 16 threaduri, chiar daca variavila globala e setata la 4, se ia variabila locala
    {
        ...
        #pragma omp for schedule(static, 2) collapse(2) // collapse(2) - linearizeaza primele 2 for-uri si dupa le paralelizeaza, daca nu il aveam doar primul for se paraleliza by default
        for (int i = 0; i < MAX; i++) {
            for (int j = 0; j < MAX; j++) {
