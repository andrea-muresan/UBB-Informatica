...
int MDO::diferentaValoareMaxMin() const
{
	...
	TValoare max = this->elems[current].elems[this->elems[current].prim].second;
	TValoare min = this->elems[current].elems[this->elems[current].prim].second;;
	...
	while (current != -1)
	{
		...
		while (first_elem != -1)
		{
			if (this->elems[current].elems[first_elem].second < min)
				min = this->elems[current].elems[first_elem].second;
			if (this->elems[current].elems[first_elem].second > max)
				max = this->elems[current].elems[first_elem].second;
			first_elem = this->elems[current].urm[first_elem];
		}
		current = urm[current];
	}
