...
void testColectiaValorilor() {
	...
	// 5:5 1:1 10:10 7:7 -3:-3
	std::vector<TValoare> vector_valori = d.colectiaValorilor();
	assert(vector_valori.size() == 5);
	assert(vector_valori[0] == 5);
	assert(vector_valori[2] == 10);
	assert(vector_valori[4] == -3);
}
